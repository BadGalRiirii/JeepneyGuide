  ...
  return (
    <button
      ...
      className={`w-full text-left flex items-center gap-3 px-3 py-3.5 rounded-2xl transition-all active:scale-[0.98] ${
        selected
          ? 'bg-th-glassh border border-th-border'
          : 'hover:bg-th-glass active:bg-th-glassh border border-transparent'
      }`}
    >
      <div
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-1.5 flex-wrap">
          <span className="font-semibold text-th-text text-[15px] leading-snug truncate">
            {route.name}
          </span>
          {nearby && (
            <span className="flex-shrink-0 text-[10px] px-1.5 py-0.5 rounded-full bg-blue-500/20 text-blue-600 dark:text-blue-300 font-semibold">
              Nearby
            </span>
          )}
        </div>
        <div className={`text-xs font-medium mt-0.5 truncate ${terminal.text} opacity-80`}>
          → {terminal.short}
        </div>
      </div>

      <div className="flex-shrink-0 text-right pl-1">
        <div className="text-amber-500 font-black text-sm">₱{fare}</div>
        <div className="text-th-text3 text-[10px] mt-0.5">{km.toFixed(1)} km</div>
      </div>
    </button>
  )
}
