...
export default function GetMeThere({ onRouteSelect }) {
  ...
  // Resolve a typed string to an exact stop name
  ...
  function handleSearch() {
    ...
    setResult(findRoutes(origin, dest))
    setSearched(true)
  }
    ...
    <div className="flex flex-col gap-3 px-3 py-2">
      ...
      <div className="flex flex-col gap-1">
        ...
        <div className="relative">
          <span className="absolute left-3 top-1/2 -translate-y-1/2 text-[10px] font-black uppercase tracking-widest text-emerald-400/70 pointer-events-none select-none w-10">
            FROM
          </span>
          <input
            list="gmt-from-list"
            ...
            className="w-full bg-white/5 border border-white/10 rounded-xl pl-14 pr-4 h-11 text-sm text-white placeholder-white/20 focus:outline-none focus:border-emerald-500/40 focus:bg-white/8 transition-all"
          />
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={swap}
            className="w-8 h-8 flex items-center justify-center rounded-full bg-white/8 hover:bg-white/15 active:scale-90 border border-white/10 transition-all"
            title="Swap from/to"
          >
            <ArrowRightLeft className="w-3.5 h-3.5 text-white/50" />
          </button>
          <div className="flex-1 h-px bg-white/5" />
        </div>
        ...
        {/* TO */}
        <div className="relative">
          <span className="absolute left-3 top-1/2 -translate-y-1/2 text-[10px] font-black uppercase tracking-widest text-amber-400/70 pointer-events-none select-none w-6">
            TO
          </span>
          <input
            list="gmt-to-list"
            ...
            className="w-full bg-white/5 border border-white/10 rounded-xl pl-9 pr-4 h-11 text-sm text-white placeholder-white/20 focus:outline-none focus:border-amber-500/40 focus:bg-white/8 transition-all"
          />
      ...
      {searched && !result && (
        <div className="text-center py-5">
          ...
          <p className="text-white/50 text-sm font-medium">No route found</p>
  ...
  if (option.type === 'direct') {
    ...
    return (
      <button
        ...
        className="w-full text-left p-3 rounded-2xl bg-white/5 border border-white/10 hover:bg-white/8 active:scale-[0.98] transition-all"
      >
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            <div className="font-semibold text-white text-sm leading-snug">{option.route.name}</div>
            <div className={`text-xs mt-0.5 ${t.text} opacity-75`}>→ {t.short}</div>
            <div className="text-[11px] text-white/30 mt-0.5">{option.distanceKm.toFixed(1)} km</div>
          </div>
          <div className="flex-shrink-0 text-right">
            <div className="text-amber-400 font-black text-base">₱{option.fare}</div>
            <div className="text-white/40 text-[11px] flex items-center gap-0.5 justify-end mt-0.5">
    ...
    >
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="flex-1 min-w-0 text-center">
          <div className="text-white/30 text-[10px] uppercase tracking-widest">transfer at</div>
          <div className="text-white/70 text-xs font-semibold truncate">{option.transferStop}</div>
        </div>
      ...
      <div className="flex items-end justify-between mt-2">
        <div className="text-[11px] text-white/35 leading-snug flex-1 min-w-0 pr-2 truncate">
          {option.route1.name} → {option.route2.name}
        </div>
        <div className="flex-shrink-0 text-right">
          <div className="text-amber-400 font-black text-base">₱{option.fare}</div>
          <div className="text-white/40 text-[11px] flex items-center gap-0.5 justify-end">
            <Clock className="w-3 h-3" />{option.time} min
          </div>
        </div>
      </div>
      <div className="mt-1.5 flex items-center gap-1 text-[10px] text-white/20">
